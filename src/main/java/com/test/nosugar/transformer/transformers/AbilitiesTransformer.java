...
public class AbilitiesTransformer implements ITransformerModule {
    ...
    private static final String FIELD_PHASE_INTERNAL = "com/test/nosugar/transformer/event/AbilitiesFieldEvent$FieldPhase";
    ...
    private void dumpInsnContext(String className, MethodNode method, AbstractInsnNode target, String label) {
        ...
        for (int i = start; i < end; i++) {
            ...
            if (insn.getOpcode() >= 0) {
                sb.append(com.test.nosugar.transformer.AsmUtil.getOpcodeName(insn.getOpcode()));
            } else {
